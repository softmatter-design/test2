			...
			\begin{itemize}
				...
				\item アプローチ
					\begin{itemize}
						\item 実験的アプローチ
						\begin{itemize}
							\item 構造明確なネットワークを超分子ネットワークで構築
							\item フィラー無添加での\alert{}{高い破断伸びと強度}
							\item 既知のモデルとの多数の整合点と、よくわからない点。
						\end{itemize}
...
\begin{frame}
	...
	\begin{center}
		\begin{alertblock}<2>{}
			\alert{「素抜け鎖」でのファントムネットワークはすでに報告。}
		\end{alertblock}
	\end{center}
\end{frame}
...
\begin{frame}
	\frametitle{本発表の内容}
		\begin{block}{すぬけ鎖の振り返り}
			\begin{itemize}
				\item ランダムネットワーク作成のプロセス
				\item ランダムネットワークモデルの特徴の検討
				\begin{itemize}
					\item ファントムネットワークモデルの確認
					\item ネットワークの力学的応答
				\end{itemize}
			\end{itemize}
		\end{block}
		\begin{alertblock}{KG 鎖のランダムネットワークでの検討}
			\begin{itemize}
				\item ランダムネットワークモデルの構築
				\item その力学的及び緩和挙動の明確化。
				\item 絡み合いの影響を確認
			\end{itemize}
		\end{alertblock}
\end{frame}
...
\section{「す抜け鎖」のランダムネットワークシミュレーション}
\subsection{アプローチ}
\begin{frame}
		...
		\begin{exampleblock}{アルゴリズム}
			\begin{enumerate}
				...
				\item トポロジー空間でランダム性の導入
					\begin{itemize}
						\item ラプラシアン行列で\alert{全体の連結性を確認}しながら、
						\item \alert{エッジ交換}して、ランダム性を導入
					\end{itemize}	
				\item 対応する実空間でのネットワーク初期構造作成
			\end{enumerate}
		\end{exampleblock}
...
\subsection{平衡状態での振る舞い}
\begin{frame}
	\frametitle{初期構造の緩和}
		\begin{block}{KG 鎖をストランドとするネットワークの作成}
			\begin{itemize}
				\item 非結合ポテンシャルは LJ ポテンシャル $U_{LJ}(r_{ij})$によりビーズ間に斥力相互作用（$r_c = 2^{(1/6)}\sigma$）
				\item ボンドポテンシャルには FENE-LJ ポテンシャル
			\end{itemize}
		\end{block}
		\vspace{-3mm}
		\begin{columns}[T, onlytextwidth]
				...
				\begin{exampleblock}{初期構造の緩和}
					\begin{itemize}
						\item Auhl 等の方法\footnote{
							\scriptsize{R. Auhl et al. Journal of Chemical Physics 119, 12718 (2003)}
						}に従い、
						\begin{itemize}
							\item force-capped-LJ ポテンシャルを用いた 
							\item Slow Push Off により初期構造を緩和
						\end{itemize}
					\end{itemize}
					\vspace{-3mm}
					\scriptsize
	...
	\frametitle{ランダムネットワークの絡み合い解析}
		...
		\begin{columns}[T, onlytextwidth]
			...
			\begin{block}{仮想的なモデル状態}
				...
				\end{itemize}
				\begin{center}
					\includegraphics[width=.8\textwidth]{PPA_sunuke_NW.png}
				\end{center}
			\end{block}
		\end{columns}
		\vspace{-2mm}
...
\begin{frame}
	\frametitle{絡み合いを低減したネットワーク}
		\begin{columns}[T, onlytextwidth]
			\column{.48\linewidth}
				\begin{itemize}
					\item \alert{PPA} for 4-Chain-NPT
					
					\includegraphics[width=.8\textwidth]{NPT_PPA.png}
					\item 4-Chain
					
					\includegraphics[width=.4\textwidth]{N48_f4_PPA.png}
				\end{itemize}
			\column{.48\linewidth}
			\begin{block}{応力緩和関数 $G(t)$}
				\begin{itemize}
				...
				\end{itemize}
					\includegraphics<1>[width=\textwidth]{gt_NPT.png}
		...
		\begin{block}{本発表の内容}
			...
			\item ネットワーク構造の連結性にランダム性を導入
				\begin{itemize}
					...
					\item ストランド長がガウス分布するランダムネットワーク構造
				\end{itemize}
